...
For induction (threshold-triggered manifest generation from accumulated signals), see the separate [induction specification](../induction/SPEC.md).
...
## 6. Induction (Separate Specification)

Induction — the mechanism by which accumulated advisory signals produce new artifacts when convergence criteria are met — is specified in a separate document: **[induction/SPEC.md](../induction/SPEC.md)**.

The induction layer depends on this advisory layer (requires Level 1 conformance) but is independently implementable. An advisory implementation MAY be conformant without providing induction.

See the [induction specification](../induction/SPEC.md) for: saturation specification, manifest proposals, proposal lifecycle, and compound synthesis.
...
**Input:** zone name  
**Output:** zone pressure readout + contributing signals + causal context
...
### Level 4: Induction-Capable (Separate Spec)

Induction conformance is defined in the [induction specification](../induction/SPEC.md) and validated by its own conformance expectations. An implementation claiming induction capability MUST additionally conform to `induction/conformance/expectations.yaml`.
